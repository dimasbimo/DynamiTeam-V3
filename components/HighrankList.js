      ...
      {entries.map((e, i) => {
        ...
        return (
          <Link
            ...
            className={`flex items-center gap-3 rounded-xl px-3 py-3 border transition-colors hover:bg-white/[0.04] ${top ? 'border-amber-500/30 bg-amber-500/[0.04]' : 'border-[color:var(--dyn-border)] bg-white/[0.01]'}`}
          >
            <div className="w-7 flex justify-center shrink-0"><RankBadge rank={rank} /></div>
            <AvatarRing name={e.member.nama} src={e.member.avatarUrl} size={42} />
            ...
            <div className="text-right shrink-0">
              <div className="font-display font-bold text-amber-300 inline-flex items-center gap-1 leading-none">
                <Star className="w-3.5 h-3.5" />{e.point}
              </div>
