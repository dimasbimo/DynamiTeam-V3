...
// Riwayat manual admin tidak boleh ikut total/rata-rata activity.
// Riwayat proses mingguan tetap dihitung, walaupun punya note target activity.
function isManualHistory(h) {
  const note = String(h?.note || '').toLowerCase();
  return note.includes('manual') || note.includes('oleh admin');
}

function isActivityHistory(h) {
  return !isManualHistory(h);
}
